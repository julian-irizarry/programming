...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        /*
         * check if remainder of dividing by two is zero
         */
        NaturalNumber ncopy = new NaturalNumber1L(n);
        NaturalNumber two = new NaturalNumber1L(2);
        NaturalNumber remainder = ncopy.divide(two); //divide by 2 to get the remainder

        return remainder.isZero(); //return the remainder boolean true if zero
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber holder = new NaturalNumber1L(n);
        NaturalNumber pcopy = new NaturalNumber1L(p);

        if (!pcopy.isZero()) {
            if (isEven(p)) { // (n^(p/2))^2
                pcopy.divide(two);
                powerMod(n, pcopy, m); //freelunch(n^p/2)
                NaturalNumber ncopy = new NaturalNumber1L(n);
                n.multiply(ncopy);
            } else { // n * n^p-1
                pcopy.decrement();
                powerMod(n, pcopy, m); //freelunch(n^p-1)
                n.multiply(holder);
            }
        } else { //p is zero
            n.setFromInt(1);
        }
        n.copyFrom(n.divide(m)); //set n^p mod m to n
    }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
         ...
         * Declare variables
         */
        NaturalNumber wcopy = new NaturalNumber1L(w);
        NaturalNumber ncopy = new NaturalNumber1L(n);
        NaturalNumber one = new NaturalNumber2(1);

        /*
         * run power mod to get w^2 mod n
         */
        powerMod(w, new NaturalNumber1L(2), n);

        /*
         * run power mod to get w^(n-1) mod n
         */
        ncopy.decrement();
        powerMod(wcopy, ncopy, n);

        return (w.equals(one) || !wcopy.equals(one));
    }
    ...
    public static boolean isPrime1(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
             ...
             */
            isPrime = !isWitnessToCompositeness(new NaturalNumber1L(2), n);
        }
        return isPrime;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        NaturalNumber randomWitness = new NaturalNumber1L();
        NaturalNumber ncopy = new NaturalNumber1L(n);
        ncopy.subtract(new NaturalNumber2(2)); //n-2
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < 50; i++) {
                while (true) {
                    randomWitness.copyFrom(randomNumber(ncopy));
                    if (randomWitness.compareTo(new NaturalNumber2(1)) > 0) {
                        break;
                    }
                }

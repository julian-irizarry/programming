...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        if (str1 != null && str2 != null && 0 <= overlap
                && overlap <= str1.length() && overlap <= str2.length()
                && str1.regionMatches(str1.length() - overlap, str2, 0,
                        overlap)) {

            /*
             * Sub string starting at the int overlap - 1 because the index
             * starts at 0 and ends at the length of the string creating a new
             * string without the overlap
             */

            String partialString = str1.substring(0, (str1.length() - overlap));
            combination = partialString + str2; //puts the strings together
        }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        boolean sInStr = false;
        Set<String> removableSubStrings = new Set1L<>();

        /*
         * Iterate through strSet
         */
        for (String s : strSet) {
            /*
             * Check if str is a substring of any strings in strSet
             */
            if (s.contains(str)) {
                /*
                 * Leave strSet alone if true
                 */
                return;
            }
        }
        ...
        /*
         * Look for substrings to remove
         */
        for (String k : strSet) {
            if (str.contains(k)) {
                sInStr = true; //strSet.remove(k)
                /*
                 * cannot iterate and remove so when true add to set to remove
                 * all at once
                 */
                removableSubStrings.add(k);
            }
        }
         ...
         */
        if (sInStr) {
            strSet.remove(removableSubStrings);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /*
         * Continue until input is at the end
         */

        while (!input.atEOS()) {
            /*
             * look for strings and sub strings in line and if none exist then
             * add to set lines
             */
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * replaces tilde with newline
         */
        String textNewline = text.replaceAll("~", "\n");
        /*
         * outputs string with newline character
         */
        out.println(textNewline);
